...
// TODO: abstract
function hasContent(rootNode): boolean {
...
export = function(scribe: Scribe) {
     ...
     */
    scribe.el.addEventListener('keydown', function(event: KeyboardEvent) {
        if (event.keyCode === ENTER) {
            var selection = new scribe.api.Selection()
            var range = selection.range

            var blockNode = selection.getContaining((node) => node.nodeName === 'LI' || (/^(H[1-6])$/).test(node.nodeName))
